...
export class MongoDbDriverExporter {
    constructor(mongoClient: MongoClient, register: Registry) {
        ...
        const poolSize = new Gauge({
            name: "mongodb_driver_pool_size", help: "the current size of the connection pool, including idle and and in-use members", labelNames: [
                "server_address"
            ]
        });
        const minSize = new Gauge({
            name: "mongodb_driver_pool_min", help: "the minimum size of the connection pool", labelNames: [
                "server_address"
            ]
        });
        const maxSize = new Gauge({
            name: "mongodb_driver_pool_max", help: "the maximum size of the connection pool", labelNames: [
                "server_address"
            ]
        });
        const checkedOut = new Gauge({
            name: "mongodb_driver_pool_checkedout", help: "the count of connections that are currently in use", labelNames: [
                "server_address"
            ]
        });
        const waitQueueSize = new Gauge({
            name: "mongodb_driver_pool_waitqueuesize", help: "the current size of the wait queue for a connection from the pool", labelNames: [
                "server_address"
            ]
        });
        ...
        const commands = new Histogram({
            name: "mongodb_driver_commands_seconds", help: "Timer of mongodb commands", labelNames: [
                "command",
                "server_address",
                "status"
            ]
